...
## which country
<<<<<<< HEAD
countries <- c("Italy", "Germany", "United Kingdom", "France", "US", "Netherlands", "France")
...
for (ci in seq_along(countries)) {
    ...
    if (length(country_ts_ind) == 0) {
        ...
        if (length(country_ts_ind) > 1) {
            message("\nfound several country ", ci, " \"", country, "\" countries: \"", 
        ...
        for (provincei in seq_along(country_ts_ind)) {
            tmp1 <- unlist(ts_confirmed[country_ts_ind,ts_data_col_inds])
            attr(tmp1, "names") <- NULL
            tmp2 <- unlist(ts_deaths[country_ts_ind,ts_data_col_inds])
            attr(tmp2, "names") <- NULL
            tmp3 <- unlist(ts_recovered[country_ts_ind,ts_data_col_inds])
    ...
    for (fi in seq_along(report_files)) {
        ...
        if (length(country_report_ind) == 0) {
            ...
            message("found several country ", ci, " \"", country, "\" countries: \"", 
    ...
    ## plot ts
    if (!is.null(ts)) { # if reading ts was successfull
        ...
        for (ploti in seq_len(nplots)) {
            ...
            plotpath <- country
            dir.create(plotpath)
            plotname <- paste0(plotpath, "/", country, "_", gsub(" ", "_", ylab), 
            ...
            if (add_lm_log_to_plot) {
                le_text <- c(le_text,
                             ...
                             eval(substitute(expression(paste("exponential prediction (doubling time = [", estimate, " ", 
                                                              ts_dt_unit, "]"^paste(-1), " = ", doubling_time, " ",
                                                              ts_dt_unit, ")")),
...
>>>>>>> c4a205df9849f0c2482f0f6b7fea96a1d0f1726c
